...
def count_by_values_features(df, colname, values):
    for i, value in enumerate(values):
        tmp_df = df.group_by('id').agg(pl.col(colname).is_in([value]).sum().alias(f'{colname}_{i}_cnt'))
        df = df.join(tmp_df, on='id', how='left')
    return df

def input_words_stats_features(df):
    ...
    df = df.join(temp, on='id', how='left')
    return df

def numerical_columns_features(df):
...
def categorical_columns_features(df):
...
def word_feats(df):
    ...
    df = df.merge(word_agg_df, on='id', how='left')
    return df
...
def sent_feats(df):
    ...
    df = df.merge(sent_agg_df, on='id', how='left')
    return df
...
def parag_feats(df):
    ...
    df = df.merge(paragraph_agg_df, on='id', how='left')
    return df

def get_keys_pressed_per_second(df):
    logs = df['logs']
    ...
    # wild guess
    logs = logs.merge(temp_df[['id', 'keys_per_second']], on='id', how='left')

    df['logs'] = logs  # Update the logs DataFrame in the combined_data
    return df

def product_to_keys(df):
    essays, logs = df['essays'], df['logs']
